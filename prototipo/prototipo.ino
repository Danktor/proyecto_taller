...
const int medida = 100; //distancia a las personas 
...
unsigned long tiempoUltimaDeteccion1 = 0; // Tiempo de la última detección para sensor 1
unsigned long tiempoUltimaDeteccion2 = 0; // Tiempo de la última detección para sensor 2
const unsigned long retardoDeteccion = 1000; // 1 segundo de espera entre detecciones
...
void loop() {
    ...
    if((tiempoActual - tiempoUltimaDeteccion1 > 5000)&&(distanceCm_1anterior==1)&&(distanceCm_2anterior==0)){
      distanceCm_1anterior = 0;
      Serial.println("Borro el sensor 1/////");
    }
    if((tiempoActual - tiempoUltimaDeteccion2 > 5000)&&(distanceCm_2anterior==1)&&(distanceCm_1anterior==0)){
      distanceCm_2anterior = 0;
      Serial.println("Borro el sensor 2//////");
    }
    if (distanceCm_1 < medida && (tiempoActual - tiempoUltimaDeteccion1) > retardoDeteccion) {
        tiempoUltimaDeteccion1 = tiempoActual;
    ...
    if (distanceCm_2 < medida && (tiempoActual - tiempoUltimaDeteccion2) > retardoDeteccion) {
        tiempoUltimaDeteccion2 = tiempoActual;
